...
writeStyleChar = function (which) {
    if (which === '/' && openComment === false) {
        openComment = true;
        styles = document.getElementById('style-text').innerHTML + which;
    } else if (which === '/' && openComment === true) {
        openComment = false;
        styles = document.getElementById('style-text').innerHTML.replace(/(\/[^\/]*\*)$/, '<em class="comment">$1/</em>');
    } else if (which === ':') {
        styles = document.getElementById('style-text').innerHTML.replace(/([a-zA-Z- ^\n]*)$/, '<em class="key">$1</em>:');
    } else if (which === ';') {
        styles = document.getElementById('style-text').innerHTML.replace(/([^:]*)$/, '<em class="value">$1</em>;');
    } else if (which === '{') {
        styles = document.getElementById('style-text').innerHTML.replace(/(.*)$/, '<em class="selector">$1</em>{');
    } else {
        styles = document.getElementById('style-text').innerHTML + which;
    }
    document.getElementById('style-text').innerHTML = styles;
    document.getElementById('style-tag').append(which);
};
